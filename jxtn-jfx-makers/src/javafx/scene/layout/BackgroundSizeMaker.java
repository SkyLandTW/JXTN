// @formatter:off
/*
 * Unlicensed, generated by javafx.ftl
 */

package javafx.scene.layout;

/**
 * {@link BackgroundSize}建構器。
 *
 * @author JarReflectionDataLoader-1.0.0
 * @version jfxrt.jar
 * @param <Z> 要建構的物件型態(需繼承{@link BackgroundSize})
 * @param <B> 建構器本身的型態(需繼承{@link BackgroundSizeMaker})
 */
@javax.annotation.Generated("Generated by javafx.ftl")
@SuppressWarnings("all")
public class BackgroundSizeMaker<Z extends BackgroundSize, B extends BackgroundSizeMaker<Z, B>>
        extends jxtn.jfx.makers.AbstractMaker<Z, B>
        implements BackgroundSizeMakerExt<Z, B>
{

    @Override
    public void applyTo(Z instance)
    {
        super.applyTo(instance);
    }

    /**
     * 建構{@link BackgroundSize}物件。
     *
     * @return 新的{@link BackgroundSize}物件實體
     */
    @SuppressWarnings("unchecked")
    public BackgroundSize build(double arg0, double arg1, boolean arg2, boolean arg3, boolean arg4, boolean arg5)
    {
        BackgroundSize instance = new BackgroundSize(arg0, arg1, arg2, arg3, arg4, arg5);
        this.applyTo((Z) instance);
        this.doAfterBuild((Z) instance);
        return instance;
    }
}
